...
export default function TagSourcesPage() {
  ...
  const handleStartTagging = () => {
    handleTagSources(script);
  };

  const handleRetag = () => {
    handleTagSources(taggedScript ?? script);
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          <h1 className="text-2xl font-bold mb-2 flex items-center gap-2">
            <Film className="w-7 h-7 text-claude" />
            영상 소스 태깅
          </h1>
          <p className="text-zinc-400">
            완성된 대본을 붙여넣으면 단락별로 영상/이미지 소스를 태깅해줍니다.
            대본 생성 워크플로우와 별개의 독립 도구입니다.
          </p>
        </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <textarea
                ...
                placeholder="완성된 대본을 붙여넣으세요. 직접 작성한 대본, 다른 곳에서 가져온 대본, 어떤 대본이든 가능합니다."
                rows={14}
        ...
        <p className="mt-8 text-xs text-zinc-500 text-center">
          <Link href="/settings" className="text-zinc-400 hover:text-zinc-300 underline">
            설정
          </Link>
          에서 Anthropic API 키를 입력해야 사용할 수 있습니다.
        </p>
      </div>
    </div>
  );
